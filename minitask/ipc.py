...
class IPC:
    ...
    def __init__(
        ...
        *,
        ...
        transport: t.Optional[Communication] = None,
    ):
    ...
    def connect(self, endpoint: str, *, sensitive: bool = False) -> InternalReceiver:
        io = self.transport.create_reader_port(endpoint)
    ...
    def serve(self, endpoint: str, *, sensitive: bool = False) -> InternalReceiver:
        io = self.transport.create_writer_port(endpoint)
        assert io is not None, io
...
class InternalReceiver(t.Generic[T]):
    ...
    def recv(self) -> T:
        msg = self.transport.read(file=self.io)
        if not msg:
            return None
        return self.format_protocol.decode(msg)

    def __iter__(self) -> t.Iterable[T]:
        self._buffer, teardown = self.transport.create_reader_buffer(self.recv)
...
class InternalSender:
    ...
    def send(self, message: T) -> bytes:
        ...
        return self.transport.write(b, file=self.io)
    ...
    def __exit__(self, typ, val, tb):
        ...
        if typ is not None:
            ...
            self.transport.write(b, file=self.io)
